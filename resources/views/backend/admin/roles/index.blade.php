                                                ...
                                                @if($roles->count() > 0)
                                                    @forelse ($roles as $role)
                                                        <tr>
                                                            ...
                                                            <td>
                                                            @php
                                                                $configEnglishPermissions = config('permissions_en.permissions')
                                                            @endphp
                                                            @if(is_array($role->permissions))
                                                                    @foreach($role->permissions as $permission)
                                                                        @if(isset($configEnglishPermissions[$permission]))
                                                                            <span class="badge badge-primary">{{ $configEnglishPermissions[$permission] }}</span>
                                                                        @endif
                                                                    @endforeach
                                                            @endif
                                                            </td>
                                                                ...
                                                                <div style="display: inline-block;">
                                                                    <div class="dropdown">
                                                                        ...
                                                                        <div class="dropdown-menu" aria-labelledby="dropdownBreadcrumbButton">
                                                                            <a class="dropdown-item" href="{{ route('admin.roles.edit' , $role->id) }}"><i class="la la-edit"></i> {{ __('dashboard.edit') }}</a>
                                                                            <a class="dropdown-item" href="javascript:void(0);" onclick="document.getElementById('delete_form_'+{{ $role->id }}).submit();"><i class="la la-trash"></i> {{ __('dashboard.delete') }}</a>
                                                                        </div>
                                                                    </div>
                                                ...
                                                @if($roles->count() > 0)
                                                    @forelse ($roles as $role)
                                                        <tr>
                                                            ...
                                                            <td>
                                                            @php
                                                            $configArabicPermissions = config('permissions_ar.permissions')
                                                            @endphp
                                                            @if(is_array($role->permissions))
                                                                    @foreach($role->permissions as $permission)
                                                                        @if(isset($configArabicPermissions[$permission]))
                                                                            <span class="badge badge-primary">{{ $configArabicPermissions[$permission] }}</span>
                                                                        @endif
                                                                    @endforeach
                                                            @endif
                                                            </td>
